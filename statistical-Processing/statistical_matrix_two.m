...
g = [ones(size(stat_cancer)); 2*ones(size(stat_benign))];
x = [stat_cancer; stat_benign];
...
% label the plot
yMin = min([min(stat_benign)-1,min(stat_cancer)-1]);
yMax = max([max(stat_benign)+1,max(stat_cancer)+1]);
% yMin = 0; yMax = 0.3;
