...
% =====================================================================================
    ...
    g = [ones(size(cancer_arg)); 2*ones(size(benign_arg))];
    x = [cancer_arg; benign_arg];

    % plot the box plot
    figure, 
    boxplot(x,g,'labels',{'Cancer','Benign'});
    ...
    % label the plot
    yMin = min([min(benign_arg)*0.8,min(cancer_arg)*0.8]);
    yMax = max([max(benign_arg)*1.2,max(cancer_arg)*1.2]);
    % yMin = 0; yMax = 0.3;
