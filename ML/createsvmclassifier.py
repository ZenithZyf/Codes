...
def createsvmclassifier(xTr, yTr, alphas, bias, ktype, kpar):
    ...
    def svmclassify(xTe):
        d,n = xTe.shape
        ay = np.multiply(alphas, yTr)
        Ke = computeK(ktype, xTr, xTe, kpar)
        ayk = np.multiply(ay, Ke)
        fxe = np.sum(ayk, axis=0)+bias
        train_preds = np.sign(fxe.T)
        train_preds = train_preds.reshape(n,1)
        return train_preds
