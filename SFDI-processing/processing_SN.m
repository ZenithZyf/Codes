...
% please name the other folder as the diagnose name that you want to present
    ...
    load 'E:\Projects\SFDI\Sreyankar_old_codes\Muaq.mat'
    load 'E:\Projects\SFDI\Sreyankar_old_codes\Muspq.mat'
...
%         mactis   = mactis_o;
        ...
        Rd_DC1 = (mdctis./mdcref);
        Rd_DC  = Rd_DC1.*0.7530./max(max(Rd_DC1));
%         Rd_DC = (mdctis./mdcref);
%         Rd_DC = Rd_DC./max(max(Rd_DC)).*0.7530;
        % 0.2255 is the theoretical phantom Rd_AC from diffuse_equa.m
        Rd_AC1 = (mactis./macref);
        Rd_AC  = Rd_AC1.*0.2255./max(max(Rd_AC1));
%         Rd_AC = (mactis./macref);
...
%                     Rdc = Rd_DC(r,c,wl);
%                     Rac = Rd_AC(r,c,wl);
%                     col = fix((Rac-0.05)./stepsize+1);
%                     row = fix((Rdc-Rac-stepsize)./stepsize+1);
% 
...
%         end
        ...
        for k = 1:row_image
            for j = 1:column_image
                ...
                if((row>0 && row <= size(Muaq,1))&&(column > 0 && column <= size(Muaq, 2)))
                    ...
                    Mua_result(k,j,wl) = Mua/2;
                    Musp_result(k,j,wl) = Musp*4;
                else
                    beyond_counter = beyond_counter + 1;
                end
            end
        ...
        Mua_savename  = fullfile(folder_name,category_case(i),'MuaSN.mat');
        Musp_savename = fullfile(folder_name,category_case(i),'MuspSN.mat');
